    ...
    <div className={`flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-100 dark:border-slate-700/60 shadow-sm mb-6 ${className}`}>
      <div className="space-y-1">
        ...
        {breadcrumbs.length > 0 && (
          <nav className="flex items-center space-x-1.5 text-[10px] font-bold text-slate-400 dark:text-slate-505 uppercase tracking-wider mb-1">
        ...
        <h2 className="text-xl font-extrabold text-slate-800 dark:text-slate-105 tracking-tight">
          {title}
        </h2>
        {description && (
          <p className="text-xs text-slate-400 dark:text-slate-505 leading-normal font-semibold mt-0.5">
            {description}
          </p>
        )}
      </div>
