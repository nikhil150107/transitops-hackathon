...
const StatusBadge = ({
  ...
  const getBadgeStyle = () => {
    const value = (status || children || '').toLowerCase().trim();
    ...
    if (
      ...
      return 'bg-rose-50 dark:bg-rose-500/10 text-rose-600 dark:text-rose-455 border border-rose-100 dark:border-rose-900/30';
    }
    if (
      value === 'off duty' || 
      value === 'draft' || 
      ...
      return 'bg-slate-55 dark:bg-slate-700/30 text-slate-500 dark:text-slate-400 border border-slate-105 dark:border-slate-800';
    }
    ...
    const colorVariants = {
      ...
      slate: 'bg-slate-55 dark:bg-slate-700/30 text-slate-600 dark:text-slate-400 border border-slate-105 dark:border-slate-800',
    };
  ...
  return (
    <span
      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[9px] font-bold tracking-wider uppercase transition-all duration-150 relative ${getBadgeStyle()} ${className}`}
      {...props}
    >
      {isPulse && (
